...
def answer_two():
    ...
    a = df[df['Gold'] - df['Gold.1'] == x].index.tolist()
    return a[0]
answer_two()
...
def answer_three():
    ...
    a = (abs(x['Gold'] - x['Gold.1'])/x['Gold.2'])
    return a.idxmax()
answer_three()
...
def answer_five():
    ...
    b = a.groupby('STNAME').count()['COUNTY']
    ans = b.idxmax()
    return ans
...
def answer_six():
    ...
    ans = b.groupby('STNAME').sum().sort_values(by='CENSUS2010POP').head(3).index.tolist()
    return ans
answer_six()
